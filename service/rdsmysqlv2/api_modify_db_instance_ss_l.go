...
func (c *RDSMYSQLV2) ModifyDBInstanceSSLCommonWithContext(ctx byteplus.Context, input *map[string]interface{}, opts ...request.Option) (*map[string]interface{}, error) {
	req, out := c.ModifyDBInstanceSSLCommonRequest(input)
	req.SetContext(ctx)
...
// In the future the SDK may create sub-contexts for http.Requests. See https://golang.org/pkg/context/
// for more information on using Contexts.
func (c *RDSMYSQLV2) ModifyDBInstanceSSLWithContext(ctx byteplus.Context, input *ModifyDBInstanceSSLInput, opts ...request.Option) (*ModifyDBInstanceSSLOutput, error) {
	req, out := c.ModifyDBInstanceSSLRequest(input)
	req.SetContext(ctx)
	req.ApplyOptions(opts...)
	return out, req.Send()
