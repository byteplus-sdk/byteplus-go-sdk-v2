...
const (
	defaultFormat = `{{.Service}}{{.Region}}.{{.SiteStack}}.com{{.CNSuffix}}`
)
...
func NewStandardEndpointResolver() *StandardEndpointResolver {
	return &StandardEndpointResolver{
		...
		Variables: StandardEndpointResolverVariable{
			Service:   "",
			Region:    "",
			SiteStack: SiteStackByteplusIPv4,
			Extension: nil,
		},
	}
}
...
var ServiceInfos = map[string]ServiceInfo{
	...
	"kickart":                     {Service: "kickart", IsGlobal: true},
	"vs":                          {Service: "vs", IsGlobal: true},
}
...
const (
	SiteStackByteplusIPv4      = "byteplusapi"
	SiteStackByteplusDualStack = "byteplus-api"
)
...
func (r *StandardEndpointResolver) EndpointFor(service, region string,
	...
	if !regionMatcher.Validate(region) {
		return resolvedEndpoint, bytepluserr.New("InvalidRegion",
	...
	if !ok {
		...
		if !ok {
			return resolvedEndpoint, bytepluserr.New("ServiceNotFound",
	...
	if ipVersion == IPVersionDualStack {
		r.Variables.SiteStack = SiteStackByteplusDualStack
	} else if ipVersion == IPVersionIPv4Only {
		r.Variables.SiteStack = SiteStackByteplusIPv4
	}

	// Go China region
	if !isGlobal && isGoChina(region) {
		r.Variables.CNSuffix = cnSuffix
	}

	t, err := template.New("endpoint").Parse(r.Format)
	if err != nil {
		return resolvedEndpoint,
			bytepluserr.New("TemplateParseError",
	...
	if err != nil {
		return resolvedEndpoint,
			bytepluserr.New("TemplateExecuteError",
...
func isGoChina(region string) bool {
