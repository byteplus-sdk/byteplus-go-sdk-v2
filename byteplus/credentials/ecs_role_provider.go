...
func (p *EcsRoleProvider) Retrieve() (Value, error) {
	if isIMDSDisabled() {
		return Value{ProviderName: EcsRoleProviderName},
			bytepluserr.New("EcsRoleDisabled",
				"IMDS credential retrieval is disabled via BYTEPLUS_ECS_METADATA_DISABLED=true", nil)
	}
	...
	if err != nil {
		return Value{ProviderName: EcsRoleProviderName},
			bytepluserr.New("EcsRoleExpirationParse",
...
func isIMDSDisabled() bool {
	v := os.Getenv("BYTEPLUS_ECS_METADATA_DISABLED")
...
func (p *EcsRoleProvider) getIMDSv2Token() (string, error) {
	...
	if err != nil {
		return "", bytepluserr.New("EcsRoleTokenFailed",
			"failed to get IMDSv2 token", err)
	}
	token := strings.TrimSpace(string(body))
	if token == "" {
		return "", bytepluserr.New("EcsRoleTokenEmpty",
...
func (p *EcsRoleProvider) resolveRoleName(imdsToken string) (string, error) {
	...
	if envRole := os.Getenv("BYTEPLUS_ECS_METADATA"); envRole != "" {
		return envRole, nil
	}
...
func (p *EcsRoleProvider) autoDetectRoleName(imdsToken string) (string, error) {
	...
	body, err := p.doRequestWithRetry(url, "GET", headers)
	if err != nil {
		return "", bytepluserr.New("EcsRoleDetectFailed",
	...
	if len(roles) == 0 {
		return "", bytepluserr.New("EcsRoleNotFound",
...
func (p *EcsRoleProvider) getCredentials(roleName, imdsToken string) (*imdsCredentialResponse, error) {
	...
	if err := json.Unmarshal(body, &resp); err != nil {
		return nil, bytepluserr.New("EcsRoleCredentialsParse",
			"failed to parse IMDS credential response", err)
	}

	if resp.AccessKeyID == "" || resp.SecretAccessKey == "" {
		return nil, bytepluserr.New("EcsRoleCredentialsEmpty",
