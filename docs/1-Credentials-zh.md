...
Byteplus Go SDK 支持多种认证方式，开发者可根据业务需求选择合适的方式接入。
...
| `CliProvider` | 从 `~/.byteplus/config.json` 读取 | 取决于 mode | 复用 CLI 登录/配置 |
...
func main() {
	...
	config := byteplus.NewConfig().
		...
		// 2. credentials.NewEnvCredentials() 不用传入任何参数，会从环境变量中读取：BYTEPLUS_ACCESS_KEY、BYTEPLUS_SECRET_KEY、BYTEPLUS_SESSION_TOKEN，生产环境建议使用这个
...
func main() {
	config := byteplus.NewConfig().
		...
		WithCredentials(credentials.NewStsCredentialsWithOptions(
			os.Getenv("BYTEPLUS_ACCESS_KEY"), // 子账号AK（必填）
			os.Getenv("BYTEPLUS_SECRET_KEY"), // 子账号SK（必填）
			"RoleName",                         // 扮演角色名称（必填）
			"123456",                           // 被扮演的主账号ID（必填）
			// 以下为可选配置，不传则使用默认值
			// func(o *credentials.StsAssumeRoleOptions) {
			//     o.Host = "open.byteplusapi.com" // STS 域名
...
func main() {
	...
	config := byteplus.NewConfig().
		...
		WithCredentials(credentials.NewStsCredentials(credentials.StsValue{
			AccessKey:       ak,              // 子账号AK,最好从环境变量获取：os.Getenv("BYTEPLUS_ACCESS_KEY")
			SecurityKey:     sk,              // 子账号SK，最好从环境变量获取：os.Getenv("BYTEPLUS_SECRET_KEY")
			...
			MaxRetries:    3,                 // 可选：AssumeRole 失败时的额外重试次数；<= 0 使用默认 3
...
STS AssumeRoleOIDC（Security Token Service）是Byteplus提供的临时访问凭证机制。开发者通过oidc_token在服务端调用 STS 接口获取临时凭证（临时 AK、SK 和 Token），有效期可配置，适用于安全要求较高的场景。
...
func main() {
	p := credentials.NewOIDCCredentialsProviderWithOptions(
		...
		func(o *credentials.OIDCProviderOptions) {
			// o.RoleSessionName = ""                         // env: BYTEPLUS_OIDC_ROLE_SESSION_NAME（可选）
			// o.Policy = ""                                  // env: BYTEPLUS_OIDC_ROLE_POLICY（可选）
			// o.Endpoint = ""                                // env: BYTEPLUS_OIDC_STS_ENDPOINT（可选）
			// o.DurationSeconds = 3600                       // 有效期，默认 3600
			// o.MaxRetries = byteplus.Int(3)                  // 可选：失败时额外重试次数；nil 默认 3，0 表示关闭重试
...
func main() {
	p := &credentials.OIDCCredentialsProvider{
		OIDCTokenFilePath: "/path/to/oidc_token_file", // env: BYTEPLUS_OIDC_TOKEN_FILE（必填）
		RoleTrn:           "Your Role Trn",            // env: BYTEPLUS_OIDC_ROLE_TRN（必填）
		RoleSessionName:   "",                         // env: BYTEPLUS_OIDC_ROLE_SESSION_NAME（可选）
		Policy:            "",                         // env: BYTEPLUS_OIDC_ROLE_POLICY（可选）
		Endpoint:          "",                         // env: BYTEPLUS_OIDC_STS_ENDPOINT（可选）
		DurationSeconds:   3600,                       // 有效期
		MaxRetries:        byteplus.Int(3),            // 可选：失败时额外重试次数；nil 默认 3，0 表示关闭重试
		RetryInterval:     1 * time.Second,            // 可选：重试间隔；<= 0 时回退到 1s
	}
...
func main() {
	...
	p.MaxRetries = byteplus.Int(3)    // 可选：失败时额外重试次数；nil 默认 3，0 表示关闭重试
...
- Session Token: `BYTEPLUS_SESSION_TOKEN` (可选)
...
- 配置文件路径优先级：构造参数 > `BYTEPLUS_CLI_CONFIG_FILE` 环境变量 > `~/.byteplus/config.json`
- Profile 优先级：构造参数 > `BYTEPLUS_PROFILE` > `BYTEPLUS_CLI_PROFILE` > 配置中的 `current` > `default`
...
- 角色名优先级：构造参数 > `BYTEPLUS_ECS_METADATA` 环境变量 > 从 IMDS 自动检测
...
1. **EnvProvider** — 环境变量（`BYTEPLUS_ACCESS_KEY` / `BYTEPLUS_ACCESS_KEY_ID`）
2. **OIDCCredentialsProvider** — 从环境变量读取 OIDC（`BYTEPLUS_OIDC_TOKEN_FILE`、`BYTEPLUS_OIDC_ROLE_TRN`）
3. **CliProvider** — CLI 配置文件（`~/.byteplus/config.json`）
...
> 1. **环境变量**：`BYTEPLUS_ACCESS_KEY` / `BYTEPLUS_SECRET_KEY`（见 [EnvironmentVariables-zh.md](EnvironmentVariables-zh.md)）
> 2. **CLI 配置文件**：`~/.byteplus/config.json`（参见上文 *CLI 配置文件凭证提供者*）
> 3. **默认凭证链**：不显式配置时由 SDK 自动发现（参见上文 *默认凭证提供者*）

`~/.byteplus/credentials` 共享凭据文件（INI 格式，AWS 风格）继续支持，路径与字段语义未变。在新的默认凭证链中，该文件不再作为独立 provider 出现，而是由 `session` 层的 `sharedConfig` 解析路径处理——老用户的代码无需任何改动即可继续使用。

注意：本文件与 CLI 凭证文件 `~/.byteplus/config.json`（JSON 格式，由 byteplus-cli 写入）**是两套独立体系**。
